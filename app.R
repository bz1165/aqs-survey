# AQS AI 使用现状摸底问卷 — Posit Connect 测试版
# 目的：最小化变量，专门测试 submit 是否成功、服务器是否断开、数据是否落盘
...
DB_PATH <- file.path(getwd(), "responses.sqlite")
...
write_response <- function(rv) {
  ...
  clean <- function(x) {
    v <- trimws(scalar(x))
    if (nchar(v) == 0) NA_character_ else v
  }
...
q7_page <- function(s = list()) {
  qcard("Q7", "多选（最多 3 项）", "你在使用 AI 过程中遇到过哪些挑战？（最多选 3 项）",
        tagList(
          ...
          conditionalPanel(
            "input.q7 !== null && input.q7.includes('H')",
            div(class = "cond-field",
...
q9_page <- function(s = list()) {
  ...
  qcard("Q9", "排序",
        ...
        rank_ui("q9_ranking", items, "其他（请注明，也可以是对上面选项的细化）：", sv(s, "q9_other", "")))
}
...
q12_page <- function(s = list()) {
  qcard("Q12", "开放问题（选填）",
        ...
        textAreaInput("q12", NULL, value = sv(s, "q12", ""), placeholder = "请在此输入…", rows = 5, width = "100%"))
}
...
thankyou_page <- function() {
  div(
    ...
    div(
      ...
      p(style = "margin-top:12px;", "📅 从下周开始每周三 11:30 在 Teams 频道发布 AQS AI Weekly Tip，欢迎留意。")
    )
  )
}
...
server <- function(input, output, session) {
  ...
  output$progress_ui <- renderUI({
    ...
    p <- page()
    if (p %in% c(0, THANKYOU_PAGE)) return(NULL)
    pct <- round(((p - 1) / (LAST_Q_PAGE - 1)) * 100)
  ...
  output$diag <- renderPrint({
    df_now <- tryCatch(load_all_responses(), error = function(e) data.frame())
    list(
      getwd = getwd(),
      normalized_app_dir = normalizePath(".", winslash = "/", mustWork = FALSE),
      tempdir = tempdir(),
      db_path = DB_PATH,
      db_exists = file.exists(DB_PATH),
      can_write_app_dir = file.access(".", 2) == 0,
      db_size_bytes = if (file.exists(DB_PATH)) file.info(DB_PATH)$size else NA,
      response_count = if (nrow(df_now) > 0) nrow(df_now) else 0,
      files_in_app_dir = list.files(".", all.files = TRUE)
    )
  })
    ...
    if (is_admin()) {
      ...
      return(
        div(
          ...
          div(
            class = "info-card",
            p(tags$strong(paste0("当前共收到 ", n, " 份回答")), style = "font-size:16px; margin-bottom:16px;"),
            if (n == 0) {
  ...
  observeEvent(input$btn_next, {
    ...
    if (!v$ok) {
      showNotification(v$msg, type = "warning", duration = 4)
      return()
    }
    ...
    if (p == LAST_Q_PAGE) {
      ...
      message("DB_PATH exists before write: ", file.exists(DB_PATH))
      message("App dir writable: ", file.access(".", 2) == 0)

      ok <- tryCatch({
        write_response(rv)
      }, error = function(e) {
        message("SUBMIT_ERROR: ", conditionMessage(e))
        FALSE
      })

      message("SUBMIT_AFTER_WRITE: ", ok)
