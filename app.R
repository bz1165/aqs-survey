...
# ── 自定义控件（中文标签走 tags$span 路径，不经过 radioButtons 的内部处理）──────
radio_hc <- function(inputId, choices_cn, selected = character(0)) {
  labels <- enc2utf8(names(choices_cn));  values <- unname(choices_cn)
  items  <- Map(function(lbl, val) {
    chk <- if (val %in% selected) list(checked = NA) else list()
    tags$div(class = "radio", tags$label(
      do.call(tags$input, c(list(type="radio", name=inputId, value=val), chk)),
      tags$span(lbl)))
  }, labels, values)
  div(id=inputId, class="shiny-input-radiogroup form-group shiny-input-container",
...
check_hc <- function(inputId, choices_cn, checked = character(0)) {
  ...
  items  <- Map(function(lbl, val) {
    ...
    tags$div(class = "checkbox", tags$label(
      do.call(tags$input, c(list(type="checkbox", name=inputId, value=val),
                            if (is_chk) list(checked=NA) else list())),
      tags$span(lbl)))
  }, labels, values)
  div(id=inputId, class="shiny-input-checkboxgroup form-group shiny-input-container",
      tags$label(class="control-label", id=paste0(inputId,"-label"), `for`=inputId),
      div(class="shiny-options-group", tagList(items)))
}
...
q0_page <- function(s=list()) {
  qcard("Q0","单选","你的当前角色是？",
        radio_hc("q0", enc2utf8(c("A. 统计师"="A","B. 统计程序员"="B",
                                   "C. 组长 / 部门管理者"="C","D. 其他角色"="D")),
        ...
        tagList(
          ...
          conditionalPanel("input.q1 !== null && input.q1.includes('F')",
                           div(class="cond-field",
                               textInput("q1_other","请注明：",value=sv(s,"q1_other",""),
                                         placeholder="请输入…")))))
}
  ...
  qcard("Q2","单选","你使用最频繁的 AI 工具是？",
        tagList(
          radio_hc("q2", enc2utf8(c(
            "A. Internal ChatGPT（含嵌入的 Claude）"="A","B. M365 Copilot"="B",
            "C. Microsoft Agent"="C","D. Coach Mira"="D",
            "E. GitHub Copilot / Cursor / 其他编程类 AI"="E","F. 其他"="F",
            "G. 无 — 我目前几乎不使用 AI 工具（选此项将跳过 Q3、Q4）"="G")),
            selected=sv(s,"q2",character(0))),
          conditionalPanel("input.q2 === 'F'",
                           div(class="cond-field",
                               textInput("q2_other","请注明：",value=sv(s,"q2_other",""),
                                         placeholder="请输入…")))),
...
q4_page <- function(s=list()) {
  qcard("Q4","单选","你使用 AI 工具的总体频率是？",
        radio_hc("q4", enc2utf8(c(
          "A. 远超每日一次（没有 AI 几乎工作不下去）"="A","B. 至少每日一次"="B",
          "C. 至少每周一次"="C","D. 每月或更少"="D","E. 视具体情况而定"="E")),
  ...
  qcard("Q5","单选 + 可选补充",
        ...
        tagList(
          ...
          conditionalPanel("input.q5 === 'G'",
                           div(class="cond-field",
                               textInput("q5_other_text","请注明：",
                                         value=sv(s,"q5_other_text",""),
                                         placeholder="请输入…"))),
          div(class="other-field",
  ...
  qcard("Q6","多选（最多 3 项）","你曾尝试用 AI 完成、但效果不满意的任务是？",
        tagList(
          ...
          conditionalPanel("input.q6 !== null && input.q6.includes('F')",
                           div(class="cond-field",
                               textInput("q6_other","请注明：",value=sv(s,"q6_other",""),
                                         placeholder="请输入…"))),
          uiOutput("q6_warn")))
}
...
q8_page <- function(s=list()) {
  qcard("Q8","单选",
        ...
        radio_hc("q8", enc2utf8(c("A. 强烈同意"="A","B. 同意"="B","C. 中立"="C",
                                   "D. 不同意"="D","E. 强烈不同意"="E")),
...
write_response <- function(rv, user_id=NA_character_) {
  ...
  df <- data.frame(
    ...
    user_id=as.character(user_id%||%NA_character_),   # session$user（部署后登录用户）
    q0=scalar(rv$q0), q1=collapse(rv$q1), q1_other=clean(rv$q1_other),
    q2=scalar(rv$q2), q2_other=clean(rv$q2_other),
    q3_ranking=collapse(rv$q3), q3_other=clean(rv$q3_other),
    q4=scalar(rv$q4),
...
server <- function(input, output, session) {
  ...
  # 捕获登录用户邮箱（仅 Shinyapps.io 开启认证后有值，本地运行为 NA）
  user_id <- tryCatch({
    u <- session$user
    if (is.null(u)||length(u)==0||trimws(u)=="") NA_character_ else trimws(u)
  ...
  observeEvent(input$btn_resume_modal,{
    removeModal()
    page(as.integer(rv$.resume_page%||%1))
    scroll_top()
  })
  ...
  output$section_ui <- renderUI({
    lbl<-section_for_page(page()); if(is.null(lbl)) return(NULL)
  ...
  observeEvent(input$btn_back,{
    p <- page()
    save_page(p,input,rv)   # 保存当前页答案，re-render 时自动恢复
